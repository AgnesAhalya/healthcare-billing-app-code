...
def mark_paid_for_user(user_id):
    with get_connection() as conn:
        conn.execute("UPDATE HEALTHCARE_001_bills SET status = 'paid' WHERE user_id = ?", (user_id,))
        conn.commit()
        return conn.execute("SELECT bill_id, user_id, amount_cents, description, status FROM HEALTHCARE_001_bills WHERE user_id = ? ORDER BY bill_id LIMIT 1", (user_id,)).fetchone()
...
def list_all_patients():
    with get_connection() as conn:
        return conn.execute("SELECT u.user_id, p.display_name, substr(p.phone, 1, 2) || '****' AS masked_phone, u.user_type FROM HEALTHCARE_001_users u JOIN HEALTHCARE_001_profiles p ON p.user_id = u.user_id WHERE u.user_type IN ('inpatient', 'outpatient') ORDER BY p.display_name").fetchall()
